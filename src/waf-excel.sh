...
function waf-excel {
    ...
    python /src/create_waf_excel.py > waf_create_excel_result.txt
    waf_exit_code=${?}

    # exit code 0 - success
    if [ ${waf_exit_code} -eq 0 ];then
        waf_comment_status="Success"
        echo "waf: info: successful waf excel creation on ${waf_file_or_dir}."
        cat waf_create_excel_result.txt
        echo
    fi
    ...
    if [ ${waf_exit_code} -ne 0 ]; then
        ...
        echo "waf: error: failed excel creation on ${waf_file_or_dir}."
        cat waf_create_excel_result.txt
        echo
    fi
...
*Workflow: \`${GITHUB_WORKFLOW}\`, Action: \`${GITHUB_ACTION}\`, waf: \`${waf_file_or_dir}\`*"
    ...
    cat waf_create_excel_result.txt >> "$GITHUB_OUTPUT"
