...
function waf {
    ...
    validate_waf  ${waf_file_or_dir} > lint_result.txt
    lint_exit_code=${?}

    # exit code 0 - success
    if [ ${lint_exit_code} -eq 0 ];then
        lint_comment_status="Success"
        echo "lint: info: successful yamllint on ${waf_file_or_dir}."
        cat lint_result.txt
        echo
    fi
    ...
    if [ ${lint_exit_code} -ne 0 ]; then
        ...
        echo "lint: error: failed yamllint on ${waf_file_or_dir}."
        cat lint_result.txt
        echo
    fi
    ...
    if [ "${GITHUB_EVENT_NAME}" == "pull_request" ] && [ "${yamllint_comment}" == "1" ] && [ ${lint_exit_code} -ne 0 ]; then
        lint_comment_wrapper="#### \`waf\` ${lint_comment_status}
...
*Workflow: \`${GITHUB_WORKFLOW}\`, Action: \`${GITHUB_ACTION}\`, Lint: \`${waf_file_or_dir}\`*"
